...
void GPS::process()
{
    ...
    else if (tokens[0] == "$GPGGA")
    {
        ...
        try
        {
            state.lla[0] = int32_t(std::stoi(tokens[2])/100) + std::fmod(std::stod(tokens[2]), 100)/60.0;
            if (tokens[3] == "S") { state.lla[0] *= -1; }
            state.lla[1] = int32_t(std::stoi(tokens[4])/100) + std::fmod(std::stod(tokens[4]), 100)/60.0;
    ...
    else if (tokens[0] == "$GPGLL")
    {
        ...
        emit<Scope::NETWORK, Scope::LOCAL>(std::make_unique<message::sensor::GPSRaw>(state), "", true);
    }
