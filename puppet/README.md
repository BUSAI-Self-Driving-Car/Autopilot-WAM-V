...
Configuration management for the RobotX team.

# Directions for setting up a RobotX development VM:
...
1.	Install Ubuntu 12.04 on a new VirtualBox VM:
	...
	* (optional) Shared Folders -> Add: Name 'robotx', Path '/Users/mitchell/robotx'. (for example, on OSX)
	...
	Start the machine: (Select `ubuntu-12.04.3-desktop-i386.iso` as the installation media)
		...
		8. Set up location + language + keyboard + user + password however you want!
			Note: RobotX' go-to settings are:
			* Sydney
			* US English
			* US English
			* nubot
...
4. Clone the git repo into `/etc/puppet`:
		...
		$ sudo git clone https://github.com/mitchellmetcalfe/robotx-puppet.git /etc/puppet
